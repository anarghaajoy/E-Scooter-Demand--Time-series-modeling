...
# Importing data
setwd("/Users/roopeshraveendran/Documents/01 Documents/02 Anargha/MS/Sixth Quarter/Capstone 2/Code")
demand_supply <- read.csv("demand_supply.csv")
head(demand_supply)

#Importing community name
community_df <- read.csv("Community_name.csv")
head(community_df)
...
# Getting community name on to demand_supply tibble
demand_supply <- demand_supply %>%
    left_join(community_df, c("Community" = "Community_number"))
...
ui <- dashboardPage(skin = 'green',
    ...
    dashboardBody(
        ...
        fluidRow(
            box(selectInput("Community_name", "Select Community", choices = unique(demand_supply$Community_name)), width = 4),
...
server <- function(input, output) { 
    ...
    selected_df <- reactive({
        demand_supply %>%
            filter(Community_name == input$Community_name & start_date == input$start_date & start_time == input$start_time)
    })
    ...
    output$barplot <- renderPlot({
        demand_supply %>%
            filter(Community_name == input$Community_name & start_date == input$start_date) %>%
